...
const liveUpdate = window.LIVE_UPDATE || null;
...
if (liveUpdate) {
  document.querySelector(".yesterday-label").innerHTML = `<i></i> Live today · ${liveUpdate.date}`;
  document.querySelector("#yesterday-title").textContent = liveUpdate.title;
  document.querySelector(".yesterday-copy p").textContent = "MaltaToday’s current live coverage is underway. Follow the linked report for updates from Hall 22.";
  document.querySelector(".yesterday-copy a").href = liveUpdate.sourceUrl;
  document.querySelector(".yesterday-copy a").textContent = "Follow MaltaToday live ↗";
  document.querySelector(".yesterday-number").textContent = "LIVE";
}
...
function renderRail(filter="all"){const visible=summaries.filter(d=>filter==="all"||d.type===filter);rail.innerHTML=`<div class="rail-scroll-hint"><span>Browse sittings</span><b>Scroll ↓</b></div>`+visible.map(d=>`<button class="day-button" data-day="${d.day}"><b>${String(d.day).padStart(2,"0")}</b><span>${d.date}<br>${d.title}</span></button>`).join("");rail.querySelectorAll("button").forEach(b=>b.onclick=()=>renderDay(summaries.find(day=>day.day===+b.dataset.day)));renderDay(visible[0])}
...
const nodes=[
 {id:"Daphne Caruana Galizia",role:"victim / journalist",bio:"Investigative journalist assassinated by a car bomb on 16 October 2017. This trial concerns the prosecution allegation that Yorgen Fenech commissioned and financed her murder.",x:550,y:85,core:true},{id:"Yorgen Fenech",role:"accused",bio:"Businessman accused of complicity in Caruana Galizia’s assassination, including allegedly commissioning and financing it. He has pleaded not guilty.",x:550,y:330,core:true},{id:"Melvin Theuma",role:"state witness / middleman",bio:"Pardoned state witness whom the prosecution identifies as the intermediary between the alleged commissioner and the killers. His recordings, money and credibility are central to the trial.",x:310,y:285,core:true},{id:"Keith Schembri",role:"former OPM chief of staff",bio:"Former chief of staff to prime minister Joseph Muscat and a longstanding associate of Fenech. He testified and denied allegations linking him to the murder; the defence has focused on his communications and missing devices.",x:800,y:270,core:true},{id:"Vince Muscat",role:"convicted participant",bio:"Known as il-Koħħu, he admitted participating in the assassination and is serving a prison sentence. He testified about surveillance, planning and claims attributed to the Degiorgio brothers.",x:220,y:500},{id:"George Degiorgio",role:"convicted participant",bio:"Convicted with his brother Alfred of carrying out the assassination. His testimony included disputed allegations about an earlier plot and about Cardona, Schembri and Gatt.",x:430,y:545},{id:"Alfred Degiorgio",role:"convicted participant",bio:"Convicted with his brother George of carrying out the assassination. In this trial he largely refused to answer questions after making allegations involving Schembri, Cardona and Gatt.",x:650,y:545},{id:"Chris Cardona",role:"former minister",bio:"Former economy minister named in disputed testimony about an alleged abandoned 2015 plot and information leaks. The reported allegations are contested and have not been established as fact in this trial.",x:900,y:460},{id:"Johann Cremona",role:"business associate",bio:"Business associate of Fenech and confidant of Theuma. He testified about conversations in which Theuma allegedly discussed the plot, payments, fear and secret recordings.",x:160,y:175},{id:"Edgar Brincat",role:"Theuma confidant",bio:"Friend and confidant of Theuma who testified about Theuma’s fears, recordings and conversations concerning money and the people he named.",x:950,y:125},{id:"David Gatt",role:"lawyer / former police inspector",bio:"Lawyer and former police inspector mentioned in testimony about the Marsa potato shed, prison visits and the alleged abandoned 2015 plot. Allegations involving him remain disputed and unproven.",x:1030,y:340},{id:"Keith Arnaud",role:"lead investigator",bio:"Assistant Police Commissioner who led the murder investigation. He guided jurors through the inquiry and was cross-examined about investigative choices, searches and missing devices.",x:80,y:370}
];
const edges=[
 ...
 ["Keith Schembri","David Gatt",12,0,"Schembri said investigators encouraged him to meet Gatt after the lawyer tried to contact him, and described Gatt seeking an ambassadorial appointment. His account was tested against phone data and earlier testimony.","Day 25"],
...
const mentionTotals=Object.fromEntries(nodes.map(n=>[n.id,0]));
for(const [from,to,count] of edges){mentionTotals[from]+=count;mentionTotals[to]+=count}
const maxMentions=Math.max(...Object.values(mentionTotals));
const nodeRadius=name=>32+Math.sqrt(mentionTotals[name]/maxMentions)*30;
...
nodes.forEach(n=>{const radius=nodeRadius(n.id),g=document.createElementNS(ns,"g");g.classList.add("node");g.setAttribute("role","button");g.setAttribute("tabindex","0");g.setAttribute("aria-label",`View ${n.id} profile`);g.innerHTML=`<circle cx="${n.x}" cy="${n.y}" r="${radius.toFixed(1)}"/><text x="${n.x}" y="${n.y-2}">${n.id.split(" ").map((w,i)=>`<tspan x="${n.x}" dy="${i?14:0}">${w}</tspan>`).join("")}</text><text class="role" x="${n.x}" y="${n.y+radius+17}">${n.role}</text>`;const selectPerson=()=>{svg.querySelectorAll(".node").forEach(node=>node.classList.remove("selected"));g.classList.add("selected");const related=edges.filter(e=>e[0]===n.id||e[1]===n.id);card.innerHTML=`<div class="section-kicker">${n.role}</div><h3>${n.id}</h3><p class="person-bio">${n.bio}</p><div class="score"><b>${mentionTotals[n.id]}</b> mapped mentions · <b>${related.length}</b> connections</div><p class="connection-list">${related.map(e=>`${e[0]} → ${e[1]} (${e[2]>10?"10+":e[2]>5?"6–10":e[2]>2?"3–5":"1–2"})`).join("<br>")}</p>`};g.onclick=selectPerson;g.onkeydown=event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();selectPerson()}};svg.append(g)});
